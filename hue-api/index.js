...
HueApi.prototype._getGroupLightStateOptions = function (groupId, stateValues) {
	...
	if (state.hasRGB()) {
		return Promise.reject(new ApiError("RGB state is not supported for groups yet"));
	}
...
HueApi.prototype._getLightStateOptions = function (lightId, stateValues) {
	...
	const promise = _setLightIdOption(options, lightId);
	if (promise) {
		return promise;
	}
	...
	if (state.hasRGB()) {
		return self.lightStatus(lightId)
